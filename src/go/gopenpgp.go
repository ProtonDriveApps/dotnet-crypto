...
//export pgp_armor_message_stream
func pgp_armor_message_stream(
	writer C.PGP_ExtWriter,
	armor_type C.uchar_t,
	out_armor_writer *C.uintptr_t,
) C.PGP_Error {
	var header string
	switch armor_type {
	case C.ARMOR_MESSAGE:
	...
	writeCloser, err := armor.ArmorWriterWithType(extWriter, header)
	if err != nil {
		return errorToPGPError(fmt.Errorf("failed to prepare armor stream: %w", err))
	}

	*out_armor_writer = C.uintptr_t(cgo.NewHandle(writeCloser))
	return errorToPGPError(nil)
}
...
//export pgp_unarmor_message_stream
func pgp_unarmor_message_stream(
	reader C.PGP_ExtReader,
	out_armor_reader *C.uintptr_t,
) C.PGP_Error {
	goReader := NewExternalReader(reader)
	goReaderBuffered := bufio.NewReader(goReader)
	armorReader, err := armor.ArmorReader(goReaderBuffered)
	if err != nil {
		return errorToPGPError(fmt.Errorf("failed to prepare unarmor stream: %w", err))
	}

	*out_armor_reader = C.uintptr_t(cgo.NewHandle(armorReader))
	return errorToPGPError(nil)
}
...
//export pgp_reader_read
func pgp_reader_read(
	r C.uintptr_t,
	buffer *C.uchar_t,
	buffer_len C.size_t,
	data_read *C.size_t,
