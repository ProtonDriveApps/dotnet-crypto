...
//export pgp_generate_session_key_for_encryption
func pgp_generate_session_key_for_encryption(
	encryption_handle *C.PGP_CEncryptionHandle,
	...
	sessionKey, err := encryptor.GenerateSessionKey()
	if err != nil {
		return errorToPGPError(fmt.Errorf("failed to generate session key: %w", err))
	}
